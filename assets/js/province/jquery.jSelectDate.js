...
var jSelectDate = {
	...
	*/
	init: function(els){ els.each(function(){
		var el = $(this);
		...
		var nYear = jSelectDate.settings.yearDefault;//elDate.getFullYear();
		var nMonth = jSelectDate.settings.monthDefault;//jSelectDate.returnMonth(elDate.getMonth());
		var nDay = jSelectDate.settings.dayDefault;//elDate.getDate();
	...
	for (var i = 1; i <= 12; i++) {
		...
		if (!isNaN(nMonth)) {
			...
			var maxDayNum = 30;
			if (nMonth == 2) {
				if (jSelectDate.isLeapYear(nYear)) { maxDayNum = 29; }
				else{ maxDayNum = 28; }
			}
			else if (jSelectDate.isLargeMonth(nMonth)) { maxDayNum = 31; }
